...
#include "generate_hiz.inl"

DAXA_DECL_PUSH_CONSTANT(GenerateHizPush, push)
...
void downsample_64x64(uvec2 local_index, uvec2 grid_index, uvec2 min_mip_size, int src_mip, int mip_count) {
    ...
    for (uint quad_i = 0; quad_i < 4; ++quad_i) {
        ...
        vec4 fetch;
        if (src_mip == -1) {
            fetch = textureGather(daxa_sampler2D(push.uses.u_src, deref(push.uses.u_globals).samplers.linear_clamp), make_gather_uv(invSize, src_i), 0);
        }
        else {
            fetch.x = imageLoad(daxa_access(image2DCoherent, push.uses.u_mips[src_mip]), min(src_i + ivec2(0,0), ivec2(min_mip_size) - 1)).x;
            fetch.y = imageLoad(daxa_access(image2DCoherent, push.uses.u_mips[src_mip]), min(src_i + ivec2(0,1), ivec2(min_mip_size) - 1)).x;
            fetch.z = imageLoad(daxa_access(image2DCoherent, push.uses.u_mips[src_mip]), min(src_i + ivec2(1,0), ivec2(min_mip_size) - 1)).x;
            fetch.w = imageLoad(daxa_access(image2DCoherent, push.uses.u_mips[src_mip]), min(src_i + ivec2(1,1), ivec2(min_mip_size) - 1)).x;
        }
        const float min_v = min(min(fetch.x, fetch.y), min(fetch.z, fetch.w));
        ivec2 dst_i = ivec2((grid_index * 16 + local_index) * 2) + sub_i;
        imageStore(daxa_image2D(push.uses.u_mips[src_mip + 1]), dst_i, vec4(min_v,0,0,0));
    ...
    }
    {
        ...
        imageStore(daxa_image2D(push.uses.u_mips[src_mip + 2]), dst_i, vec4(min_v,0,0,0));
    ...
    for (i32 i = 2; i < mip_count; ++i) {
        ...
        if (invoc_active) {
            ...
            if (dst_mip == 6) {
                imageStore(daxa_access(image2DCoherent, push.uses.u_mips[dst_mip]), ivec2(glob_wg_offset + local_index), vec4(min_v,0,0,0));
            }
            else {
                imageStore(daxa_image2D(push.uses.u_mips[dst_mip]), ivec2(glob_wg_offset + local_index), vec4(min_v,0,0,0));
            }
...
void main() {
    downsample_64x64(gl_LocalInvocationID.xy, gl_WorkGroupID.xy, (deref(push.uses.u_globals).next_lower_po2_render_target_size * 2), -1, 6);
    ...
    if (s_last_workgroup) {
        downsample_64x64(gl_LocalInvocationID.xy, uvec2(0,0), (deref(push.uses.u_globals).next_lower_po2_render_target_size * 2) >> 6, 5, int(push.mip_count - 6));
    }
}